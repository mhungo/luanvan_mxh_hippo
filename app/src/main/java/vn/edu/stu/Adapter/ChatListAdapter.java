...
public class ChatListAdapter extends RecyclerView.Adapter<ChatListAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull final ViewHolder holder, int position) {
        final ChatList user = userList.get(position);
        viewBinderHelper.bind(holder.SwipeRevealLayoutChatList, user.getChatlist_id());
        ...
        //gan id user chat
        mUserChatId = user.getChatlist_id();

        //load image username user
        loadInfo(user, holder);
    ...
    private void loadInfo(ChatList user, ViewHolder holder) {
        //Load ten va hinh dai dien
        DatabaseReference reference = FirebaseDatabase.getInstance().getReference(Constant.COLLECTION_USERS);
        reference.child(user.getChatlist_id())
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull @NotNull DataSnapshot snapshot) {
                        User user1 = snapshot.getValue(User.class);
                        if (user != null) {
                            holder.textViewUsername.setText(user1.getUser_username());
                            try {
                                Glide.with(mContext).load(user1.getUser_imageurl())
                            ...
                            holder.SwipeRevealLayoutChatList.setVisibility(View.VISIBLE);
        ...
        reference1.child(user.getChatlist_id())
                ...
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull @NotNull DataSnapshot snapshot) {
                        ...
                        intent.putExtra("user_id", user.getChatlist_id());
    ...
    private void checkStatusOnOff(ChatList user, ViewHolder holder) {
