...
public class RequestJoinGroupAdapter extends RecyclerView.Adapter<RequestJoinGroupAdapter.ViewHolder> {
    ...
    public RequestJoinGroupAdapter(Context mContext, List<User> userList, String groupPostId) {
    ...
    @NotNull
    @Override
    public RequestJoinGroupAdapter.ViewHolder onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.request_friend_item, parent, false);
        return new RequestJoinGroupAdapter.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull @NotNull RequestJoinGroupAdapter.ViewHolder holder, int position) {
        User user = userList.get(position);
        if (user != null) {

            holder.text_username_request_friend.setText(user.getUser_username());
        ...
        holder.btn_comfirm_request_friend.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                confirmFriendRequest(groupPostId, user);
        ...
        holder.btn_delete_request_friend.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                deleteRequestFriend(groupPostId, user);
            }
        });
    ...
    private void loadTimeRequest(RequestJoinGroupAdapter.ViewHolder holder, User user) {
    ...
    private void confirmFriendRequest(String groupPostId, User user) {
        String timestamp_confrim_friend = System.currentTimeMillis() + "";

        HashMap<String, String> hashMap1 = new HashMap<>();
        hashMap1.put(Constant.ROLE_UID, user.getUser_id());
    ...
    //delete request
    private void deleteRequestFriend(String groupPostId, User user) {
        DatabaseReference reference = FirebaseDatabase.getInstance().getReference(Constant.COLLECTION_INVITE_GROUP);
